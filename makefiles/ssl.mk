...
$(DIR_SSL)/k8s-worker.pem: $(DIR_SSL)/ca.pem
	$(INIT-SSL) $(DIR_SSL) k8s-worker k8s-worker \
		IP.1=127.0.0.1,DNS.3=*.*.compute.internal,DNS.4=*.ec2.internal

create-ssl: $(DIR_SSL)/ca.pem $(DIR_SSL)/k8s-admin.pem $(DIR_SSL)/k8s-apiserver.pem $(DIR_SSL)/etcd.pem $(DIR_SSL)/k8s-worker.pem
...
test-ssl:
	openssl x509 -in .ssl/k8s-admin.pem -noout -text
